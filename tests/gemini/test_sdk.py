...
class TestGeminiSDK(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("anthropic_proxy.gemini_sdk.httpx.AsyncClient")
    async def test_gemini_code_assist_parses_function_call_part(self, mock_client):
        class DummyResponse:
            status_code = 200
            reason_phrase = "OK"
            ...
            async def aiter_lines(self):
                yield 'data: {"response": {"candidates": [{"content": {"parts": [{"functionCall": {"name": "get_weather", "args": {"city": "SF"}}}]}}]}}'
                yield "data: [DONE]"
    ...
    def test_normalize_function_calls_from_part(self):
            ...
            "candidates": [
                {
                    "content": {
                        "parts": [
                            {
                                "functionCall": call.model_dump(exclude_none=True),
                            }
                        ]
                    }
                }
